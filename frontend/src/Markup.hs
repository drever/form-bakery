      ...
      rec
          ...
          newText ::  Event t (Dynamic t T.Text) <- dyn $
                            let failure err = do parseError err
                                ...
                                parsedExpression = constDyn $ Right (Var 'a') -- parseExpr <$> _inputElement_value text
                             in either failure
                                       success
                                       <$> parsedExpression
      return ()
...
expression = expression' --(fmap (fmap head)) . (snd <$> expression' >> return never)
    ...
    where expression' :: DomBuilder t m
                         => Expr
                         -> m (Event t Position)
          expression' (Call []) = let p' = PVar
                                      attrs = mempty & at "data-depth" ?~ (T.pack . show $ p')
                                    -- cfg = def :: ElementConfig EventResult t (DomBuilderSpace m)
                                   in do (t, _) <- element "div" (def & initialAttributes .~ attrs) (text "empty")
                                       -- divButton VarExpr p' (text (T.pack $ e:[]) >> return never)
                                         return $ (const p') <$> domEvent Click t
          expression' (Var e) = let p' = PVar
                                    attrs = mempty & at "data-depth" ?~ (T.pack . show $ p')
                                    -- cfg = def :: ElementConfig EventResult t (DomBuilderSpace m)
                                in do (t, _) <- element "div" (def & initialAttributes .~ attrs) (text (T.pack $ show e))
                                       -- divButton VarExpr p' (text (T.pack $ e:[]) >> return never)
                                      return $ (const p') <$> domEvent Click t
          expression' _ = blank >> return never
          -- expression' p (Call es) =
          --       let subExprs = mapM (\(i, e) -> snd $ expression' (p <> (T.pack . ("|"++) . show $ i)) e)
          --                            (zip [0..] es)
          --        in (p, divButton CallExpr p (leftmost <$> subExprs))
...
fromRectState (RectState x y w h sy _) = ((x, y), (w, h))
...
dragTest t = do
     rec
              (t1, _) <-  elAttr' "div" (mempty & at "class" ?~ "test-class"
                                           & at "style" ?~ "position: static;") $
                                elDynAttr "div" (rectSize t <$> s) blank

              s :: (Dynamic t RectState) <- foldDyn (flip updateRectState)
                           (RectState 0 0 0 0 0 False)
                           (leftmost [ uncurry BeginRect <$> domEvent Mousedown t1
                                     , uncurry EndRect <$> domEvent Mouseup t1
                                     , ScrollPage <$> domEvent Scroll t1
                                     , uncurry SizeRect <$> domEvent Mousemove t1
                                     ])
     ...
     return (fromRectState <$> s)
      ...
      where updateRectState s = \case
               BeginRect x y -> s { x = x
                                  , y = y
                                  , isDragging = True } -- RectState x y 0 0 True
               SizeRect x' y' -> case isDragging s of
                                         True -> s { width = x' - x s
                                                   , height = y' - y s }
                                         False -> s
               EndRect x' y' -> s { isDragging = False }
               ScrollPage y -> s { scrollPosition = round y }
            rectSize :: T.Text -> RectState -> Map.Map T.Text T.Text
            rectSize t s = mempty & at "style" ?~ T.intercalate ";" [
                                  "top: " <> (T.pack . show $ y s - scrollPosition s) <> "px"
                                , "left: " <> (T.pack . show . x $ s) <> "px"
                                , "width: " <> (T.pack . show . width $ s) <> "px"
                                , "height: " <> (T.pack . show .height $ s) <> "px"
                                , "position: absolute"
                                , "z-index: " <> (T.pack . show . scrollPosition $ s)
                                -- , "float: left"
                                , "background-color: #1042DB66"
